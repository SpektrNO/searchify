...
- **Optional for code symbols:** `python3` (Python AST), `node` (TypeScript/JS; uses project `typescript` when present), `dotnet` (optional C# Roslyn path; heuristic works without it)
...
Look up indexed code definitions (Python / Go / TypeScript·JS / C#; requires re-index — Python needs `python3`, TS/JS needs `node`; C# prefers `dotnet` but works with the heuristic without it).
...
Schema v2 adds vector storage. Existing keyword indexes keep working. Run `index_paths` with `"force": true` (or `searchify index --force`) once to embed all chunks. Schema v4 adds code symbol tables — re-index code trees after upgrading (MCP **0.9.3**+ for C# analyzer).
