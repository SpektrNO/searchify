...
- **Embedding quality (`opt-better-embeddings`):** default `minilm-l6-v2` is small/fast (384-d), not best-in-class retrieval—especially on non-English or nuanced corporate corpora. Backlog: selectable higher-quality models (e.g. mpnet-class / multilingual), documented re-embed, without abandoning process isolation.
