...
| Code analyze | `internal/code` | Language Analyzer plugins (Python AST worker; Go `go/parser`; TS/JS Node worker) |
...
- **Code symbols (`opt-code-symbols`):** language `Analyzer` plugins; Python AST worker (`python3` on PATH, fail-soft to text chunks); **Go** via `go/parser` in-process (`opt-code-symbols-go`); **TypeScript/JS** via Node worker (`opt-code-symbols-ts`, prefers project `typescript`, else heuristic); schema v4 `chunk_symbols` / `symbols` / `symbol_refs`; MCP `lookup_symbol` / `find_references`; `search_local` may include `symbol` / `symbol_kind`. See [ADR 001](./adr/001-code-symbols.md). Re-index `.py`/`.go`/`.ts`/`.tsx`/`.js`/`.jsx` with `--force` after upgrade. Follow-on (⬜): `opt-code-symbols-csharp`.
