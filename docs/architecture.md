...
=======
- **Embed engine adapter (`opt-embed-engine-adapter`):** today “backend” means *where* kjarni runs (`process`/`onnx`/`none`), not *which* inference stack. Backlog: pluggable embed engine (kjarni default; later ONNX Runtime / HTTP worker, etc.) selected via config, still implementing `Embedder`, so Norwegian/multilingual checkpoints can be used without forking kjarni. Re-embed story shared with model-switch safety.
- **Chunking quality (`opt-better-chunking`):** today ~3KiB paragraph packing with hard caps (`SEARCHIFY_MAX_CHUNKS_PER_FILE`, extract truncate). Weak chunk boundaries hurt both BM25 and vectors. Backlog: overlap, structure-aware splits (Markdown headings, PDF pages), tunable size; clear re-index path.
>>>>>>> Stashed changes
